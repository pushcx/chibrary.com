...
#require 'riak' ; client = Riak::Client.new(:protocol => "pbc", :pb_port => 8087) ; client.buckets.each { |b| b.keys.each { |k| b.delete k } } ; require 'redis' ; Redis.new.set 'run_id', 0 ; require 'sidekiq' ; Sidekiq::Queue.new.clear ; Sidekiq::Queue.new('thread').clear ; Sidekiq::Queue.new('month_count').clear
