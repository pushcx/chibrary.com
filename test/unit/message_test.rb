...
class MessageTest < ActiveSupport::TestCase
  ...
  def test_subject_is_reply?
    m = Message.new messages(:good), 'test', '00000000'
  ...
  def test_new_saved
    ...
    assert_equal messages(:good), m.message
  end
  ...
  def test_likely_lazy_reply_to?
    good = Message.new messages(:good), 'test', '00000000'
    lazy_reply = Message.new messages(:lazy_reply), 'test', '00000000'
    assert lazy_reply.likely_lazy_reply_to?(good)
  end

  def test_formatting_message_id
    m = Message.new messages(:formatting_message_id), 'test', '00000000'
    assert_equal "list/example/message/2008/01/id%m%d%s@example.com", m.key
  end

  class Message_test_from < Message ; public :load_from ; end
  def test_from
    m = Message_test_from.new messages(:good), 'test', '00000000'
    [
  ...
  def test_no_archive
    m = Message.new messages(:no_archive), 'test', '00000000'
    assert m.no_archive
  end

  class Message_test_add_header < Message ; public :add_header, :headers ; end
  def test_add_header
    m = Message_test_add_header.new messages(:good), 'test', '00000000'
  ...
  def test_mailing_list_in_various_places
    [:good, :list_in_to, :list_in_cc, :list_in_bcc, :list_in_reply_to].each do |fixture|
      m = Message.new messages(fixture), 'test', '00000000'
  ...
  def test_no_date
    m = Message.new messages(:no_date), 'test', '00000000'
    assert (Time.now.utc - m.date) < 1
  end

  def test_wrong_format_date
    m = Message.new messages(:wrong_format_date), 'test', '00000000'
    assert_equal Time.local(2007, 8, 7, 16, 6, 33), m.date
  end

  def test_malformed_date
    m = Message.new messages(:malformed_date), 'test', '00000000'
    assert (Time.now.utc - m.date) < 1
  end

  def test_no_subject
    m = Message.new messages(:no_subject), 'test', '00000000'
  ...
  def test_message_id
    m = Message.new messages(:no_message_id), 'test', '00000000'
