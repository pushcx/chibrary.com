...
describe SummarySetRepo do
  ...
  let(:summary_containers) {
    [
      SummaryContainer.new('s1@example.com', Summary.new('callnum1', 'From', 'Foo', now, 'Blurb')),
      SummaryContainer.new('s2@example.com', Summary.new('callnum2', 'From', 'Foo', now, 'Blurb')),
    ]
  }
  let(:serialized) {
    [
